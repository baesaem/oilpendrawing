...
export const STYLE_TIP: Record<PenStyle, string> = {
  ...
  richeon: '해칭은 면의 방향대로: 벽은 세로, 바닥은 소실점 쪽으로. 나뭇잎은 뭉게구름 윤곽을 먼저 두르고 안을 작은 고리로 채우되 위쪽은 비웁니다. 하늘은 그냥 두고, 가장자리는 완성하지 말고 흐리게 끝냅니다. 마지막에 낙관.',
  fineink: '가장 가는 펜으로 아주 천천히. 얼굴은 면이 도는 방향을 따라 선을 눕히고, 눈·코·입 둘레의 작은 톤 차부터 잡습니다. 머리카락은 흐르는 방향으로 길게 긋고 덩어리째 어둡게 묶되 몇 가닥은 흰 종이로 남기세요. 뺨의 밝은 면과 콧등은 손대지 않습니다.',
  ...
  oil: '펜 대신 유화·아크릴 연습용입니다. 큰 붓으로 어두운 색부터 면을 나눠 칠하고, 마른 뒤 작은 붓으로 밝은 색을 두껍게 얹습니다. 윤곽선은 그리지 않고 색 경계로만 형태를 잡습니다.',
  vangogh: '펜 대신 유화·아크릴·오일파스텔 연습용입니다. 면마다 붓이 흐를 방향을 먼저 정하고, 물감을 섞지 않은 채 굵고 긴 자국을 나란히 놓습니다. 이웃한 자국은 색을 조금씩 바꾸고, 형태 경계는 짙은 파랑으로 한 번 둘러 줍니다.',
