...
export const STYLE_TIP: Record<PenStyle, string> = {
  richeon: '해칭은 면의 방향대로: 벽은 세로, 바닥은 소실점 쪽으로. 나뭇잎은 뭉게구름 윤곽을 먼저 두르고 안을 작은 고리로 채우되 위쪽은 비웁니다. 하늘은 그냥 두고, 가장자리는 완성하지 말고 흐리게 끝냅니다. 마지막에 낙관과 날짜.',
  parkyongsoon: '가장 가는 펜으로 아주 천천히. 하늘은 수평선을 촘촘히 긋되 구름 자리는 비워 두고 가장자리만 성기게. 가장 어두운 곳은 검게 채워 대비를 만들고, 벽의 균열·기와·잎은 작은 잔결로 끝까지 묘사합니다. 종이 끝까지 완성.',
