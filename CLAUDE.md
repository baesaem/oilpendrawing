...
5. **담채**(`brush: 'wash'`) 는 DAP 그대로다: `washTarget`(여백 문턱 아래의 사진 색을 조금 뭉개고 채도를 1.35배 살려 종이에 곱한 목표 색.
   펜의 어둡기 지도를 쓰지 않는다 — 그러면 물빠진 회색이 된다)과 캔버스의 RGB 차이가
   큰 칸에만 붓 크기 R 의 붓 자국(`Canvas.dab`, 가장자리가 (1−d²)² 로 번지는 젖은 붓)을 방향장을 따라 얹고, 다른 면으로 넘어가면 멈춘다.
   층이 끝나면 `pigmentEdges` 가 캔버스 밝기의 경계를 조금 어둡게 해 마른 물감의 고임을 낸다. 펜은 `ink` ≥ 30 일 때만 깊은 그림자에,
   윤곽은 임계값 1.5배로 아주 센 경계만 — DAP 의 Watercolor Wet on Wet 결과(사용자 영상)에 맞춘 값이다.
