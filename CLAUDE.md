...
가이드 1~3단계는 **브라우저 Canvas 로 계산**한다 (`guide.ts` 의 `edgeMap`, `valueMap`).
API 를 쓰는 건 "드로잉 생성"과 "과정 그림 만들기" 두 가지뿐이다. 이 경계를 흐리지 않는다.
...
이력에서 불러온 `params` 는 `{ ...DEFAULT_PARAMS, ...d.params }` 로 병합한다 —
필드를 새로 추가하면 옛 레코드에 그 값이 없기 때문이다.
