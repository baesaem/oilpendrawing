...
밝기를 뭉갠 뒤 `passes` 단계(기본 5)로 나누고, 단계 k 마다 **그 단계 이상 어두운 곳 전체**에 한 방향 평행선을 긋는다. 층이 겹치므로 어두울수록
선이 여러 벌 쌓이고(선 갯수), 층마다 간격을 좁히고(가장 어두운 층은 첫 층의 1/3 — `S0·(1 − 0.66·t)`, 선 굵기의 1.5배 아래로는 안 좁힌다)
굵기를 키우며(`lw·(0.75 + 0.85·t)`), 3층부터 각도를 +52°·−41°·+88° 틀어 교차선이 된다.
...
**색도(밝기를 뺀 색)가 가장 가까운 물감색을 골라 원래 밝기로 되맞춘다** — 색 수가 줄어 그림다워지되 톤은 그대로 읽힌다.
채도가 낮은 화소는 옮기지 않는다(`strength = min(1, chroma·3.2)`) — 안 그러면 하늘·물이 원색으로 튄다.
컬러 모드에서만 쓰이고, 담채·유화의 목표 색(`washTarget`)과 펜의 획 색(`colorAt`) 양쪽에 적용된다. 팔레트 12색은 `PALETTE_12`(수채 기본 세트).
...
빛 방향, `PaintPanel` 기본 화면에는 붓 일곱(+그림붓이면 브러시 팁)과 슬라이더 넷 — 세밀함(펜화면 선 간격)·여백·진하기·선 굵기 — 뿐이고(펜화 붓은 명암 단계·선 방향 둘이 더 붙는다), 슬라이더 값 옆에는
