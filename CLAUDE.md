...
4. **윤곽** — `edgePass` 가 경계가 센 자리에서 출발해 접선을 따라 양쪽으로 긋고, 지나간 자리는 획 번호로 표시해 같은 경계를
   두 번 긋지 않는다 (자기 자국에 막히지 않도록 번호를 비교한다). 경계 세기는 세 가지의 최대다: 색 경계(세 채널 Sobel 제곱합, 잔결에서는
   `1+4·texture` 로 나눠 센 것만), 덩어리 경계(뭉갠 밝기의 그라디언트 × texture — 잎 뭉치의 뭉게구름 윤곽), 색상 경계(`chromaEdgeMag`,
   밝기를 뺀 색도의 그라디언트 — 잎끼리는 작고 지붕·기둥·노란 나무 vs 녹색은 커서 잔결 속의 구조물을 살린다). 접선은 잔결 영역에서는
   지역 방향장이 소음이므로 뭉갠 밝기의 그라디언트에 수직인 방향을 쓴다 — 이걸 안 하면 윤곽 획이 잎 속을 벌레처럼 꿈틀거린다.
...
`dirAt` 과 획의 방향 갱신도 잔결일수록 더 확실할 때만 방향장을 따른다(`0.12 + 0.5·texture`). 나뭇잎 고리선은 잔결이면서 `aniso` 가 낮은
곳(방향 없는 잔결)에서만, 첫 훑기에서만 그린다 — 둘째 훑기(그림자 교차)는 해칭으로 잎 그늘을 만든다. 지시선(`manualField`)이 가까우면 그 방향으로 끌어당기고 `man` 가중치를 함께 내보내
