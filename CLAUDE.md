...
채우기 `wash`(수채 담채, 화풍 `watercolor`)는 잉크를 최소로 쓴다: 가장 어두운 단계에만 성긴 해칭, 윤곽선은 그대로.
톤은 `washBase` 가 맡는다 — 색을 크게 뭉개고(`R = 짧은 변/120`) 밝기를 `tones` 단계로 눌러 평평한 담채를 만들고,
종이에 **곱해서**(투명 물감) 얹는다. 밝은 곳(`paperKeep` 문턱 위)은 종이를 남기고, 단계 경계는 안료가 고인 것처럼 짙게,
`jitter` 는 입자감으로 쓴다. 담채 위의 펜 선은 컬러 모드라도 잉크색 그대로다. 흑백이면 먹 담채, 세피아면 세피아 담채.
